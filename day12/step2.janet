...
(defn expand-springs [line]
  (def [springs groups] (string/split " " line))
  (string
    (string/join (seq [i :range [0 5]] springs) "?")
    " "
    (string/join (seq [i :range [0 5]] groups) ",")))

(defn get-springs [line]
  (->> line
       (expand-springs)
       (peg/match spring-parse)))
...
(defn-memo find-arrangements [springs groups]
