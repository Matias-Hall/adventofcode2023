...
(defn get-field [id line]
  (peg/match
    ~{
      :main (* ,id :s+ (some :digit))
      :digit (* (/ (<- :d+) ,scan-number) :s*)
      } line))

(def times (get-field "Time:" (first (string/split "\n" input))))
(def distances (get-field "Distance:" ((string/split "\n" input) 1)))
...
(defn main [& args]
  ...
  (for i 0 (length times)
    (def t (time-range (times i) (distances i)))
    (pp t)
    (*= result t))
  (print result))
